...
import { Loader2, Search, SlidersHorizontal, Clock, Info } from 'lucide-react';
  ...
  const filteredQuartiers = useMemo(() => {
    return quartiers
      ...
      .filter(q => {
        ...
        const categoryMatch = selectedCategories.length === 0 || selectedCategories.some(cat => q.category_tags?.includes(cat));
        const ambianceMatch = selectedAmbiance.length === 0 || selectedAmbiance.some(amb => q.ambiance_tags?.includes(amb));
  ...
  return (
    <>
      ...
      <div className="pt-20 bg-white">
                 ...
                 <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                    ...
                    <Popover>
                        <PopoverTrigger asChild>
                            <Button variant="outline" className="h-12 w-full md:col-span-2 lg:col-span-2 justify-start font-normal">
                                ...
                                <div>
                                    ...
                                    <div className="flex items-center space-x-4 mt-2">
                                        <Slider
                                            ...
                                            step={1}
                                        />
                                        <span className="text-sm font-bold w-32 text-right">{proximity[0]} {t('walkingMinutesFromJemaa')}</span>
                                    </div>
                                </div>
                                <div>
                                    ...
                                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 mt-2">
                                        {categories.map(cat => (
                                            <div key={cat} className="flex items-center space-x-2">
                                                ...
                                                <label htmlFor={`cat-${cat}`} className="text-sm capitalize">{t(cat)}</label>
                                            </div>
                                        ))}
                                    </div>
                                </div>
                                    ...
                                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 mt-2">
                                        {ambiances.map(amb => (
                                            <div key={amb} className="flex items-center space-x-2">
                                                <Checkbox id={`amb-${amb}`} checked={selectedAmbiance.includes(amb)} onCheckedChange={() => handleAmbianceChange(amb)} />
                                                <label htmlFor={`amb-${amb}`} className="text-sm capitalize">{t(amb)}</label>
                                            </div>
                                        ))}
                                    </div>
                                </div>
                                <div className="flex justify-end space-x-2">
                                    <Button variant="ghost" onClick={resetFilters}>{t('resetFilters')}</Button>
                                </div>
                            </div>
                        </PopoverContent>
                    </Popover>
        ...
        <div className="content-wrapper section-padding flex flex-col">
          <div className="h-[300px] md:h-[400px] lg:h-[500px] w-full rounded-lg overflow-hidden mb-8 shadow-lg">
            <MapContainer center={[31.6258, -7.9935]} zoom={14} scrollWheelZoom={false} className="h-full w-full">
              <TileLayer
                attribution='&copy; <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a> contributors'
                url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png"
              />
              <ChangeView center={activeQuartier ? [activeQuartier.lat, activeQuartier.lng] : [31.6258, -7.9935]} zoom={activeQuartier ? 16 : 14} />
          ...
          <div className="space-y-6">
              {loading && <div className="flex justify-center items-center h-64"><Loader2 className="w-12 h-12 text-brand-action animate-spin" /> <span className="ml-4">{t('loadingQuartiers')}</span></div>}
              ...
              {filteredQuartiers.map((quartier, index) => {
                return (
                  <motion.div
                    ...
                    className={`transition-all duration-300 rounded-lg overflow-hidden ${activeQuartier?.id === quartier.id ? 'ring-2 ring-brand-action shadow-2xl' : 'shadow-lg bg-white'}`}
                  >
                    <Card className="flex flex-col md:flex-row w-full">
                      <div className="md:w-1/3 xl:w-1/4">
                        <img src={quartier.images[0]} alt={quartier.name} className="w-full h-48 md:h-full object-cover"/>
                      </div>
                        ...
                        <CardContent className="p-6 flex-grow flex flex-col justify-between">
                            <div>
                                ...
                                <div className="flex flex-wrap gap-2 mb-4">
                                    {quartier.category_tags?.map(tag => <Badge key={tag} variant="outline" className="text-xs">{t(tag)}</Badge>)}
                                    {quartier.ambiance_tags?.map(tag => <Badge key={tag} variant="outline" className="text-xs">{t(tag)}</Badge>)}
                                </div>
                            ...
                            <div className="flex flex-col sm:flex-row gap-2 mt-4">
                                ...
                                <Button asChild variant="secondary" className="w-full sm:w-auto">
                                    <Link to={`/all-riads?quartier=${quartier.slug}`}>{t('viewGuestHousesInThisDistrict')}</Link>
                                </Button>
                            </div>
                        </CardContent>
                      </div>
