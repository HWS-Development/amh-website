...
const HeroSection = () => {
  ...
  return (
    <section className="relative h-[55vh] md:h-[calc(100vh-80px)] w-full overflow-hidden text-white">
      ...
      <div className="relative z-10 h-full flex flex-col justify-between pt-8 pb-4">
        ...
        <div className="content-wrapper flex-grow flex flex-col justify-center">
          <div className="w-full">
            ...
            >
              <h1 className="h1-style leading-tight font-bold">{slide.title}</h1>
              <p className="mt-2 md:mt-4 body-text max-w-2xl text-white/90">{slide.subtitle}</p>
            </motion.div>
            
            <motion.div 
              key={currentIndex + '_card'}
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ duration: 0.8, delay: 0.5, ease: 'easeOut' }}
              className="mt-8 p-6 md:p-8 bg-brand-ink/30 backdrop-blur-md rounded-sm w-full max-w-4xl flex flex-col md:flex-row items-start md:items-center justify-between text-left"
            >
              ...
              <div className="text-left md:text-center mt-6 md:mt-0 md:ml-8">
                ...
                <p className="text-sm uppercase tracking-wider">{t('riad')}</p>
              </div>
            </motion.div>

            <motion.div
              key={currentIndex + '_button'}
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
        ...
        <div className="flex flex-col items-center">
          ...
          <div className="absolute bottom-32 left-1/2 -translate-x-1/2 flex flex-col items-center w-full px-4">
              <div className="flex items-center space-x-2 md:space-x-4">
                 ...
                 {heroSlides.map((s, i) => (
                     <button 
                         ...
                         className={`py-2 px-3 md:px-5 rounded-full text-xs md:text-sm font-semibold transition-colors duration-300 ${currentIndex === i ? 'bg-brand-action' : 'bg-white/50'}`}
                     >
                         {s.destination}
                     </button>
                 ))}
